function [x,t,u] = conservazione_LWibrido(x0,xf,N,T,K,c,inflow,f,dfdx,dfdt,u0)
...
% Ciclo iterativo
e = ones(N,1);
A_transp = spdiags([e, -e], [1, -1], N-1, N)/(2*h);
A_diff = spdiags([-e, 2*e, -e], [-1,0,1], N-1, N)/(h^2);
I = speye(N-1,N);
...
for k = 1:K
    ...
    F = f(x(2:end), t(k));
    dFdt = zeros(N-1,1); 
    dFdt(1:end) = dfdt(x(2:end-1), t(k));
    dFdx = zeros(N-1,1);
    dFdx(1:end) = dfdx(x(2:end-1), t(k));
    
    % Assegnamento della condizione di inflow
    F(1) = F(1) + (c/(2*h) + 0.5*(c^2)*dt/(h^2))*u(1,k);
    
    % Risoluzione del problema
    u(2:end-1,k+1) = (I-c*dt*A_transp-0.5*(c*dt)^2*A_diff)*u(2:end,k) + ...
        dt*F(1:end-1) + 0.5*dt^2*(dFdt-c*dFdx);
    u(end, k+1) = (1 - c*dt/h)*u(end,k) + c*dt/h * u(end-1,k) + dt*F(end); 
end